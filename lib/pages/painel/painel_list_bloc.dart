...
class PainelListBlocState {
  ...
  updatepainelMap() {
    print('painelList.length ${painelList.length}');
    List<PainelModel> painelListTemp = List<PainelModel>();

    for (var painel in painelList) {
      if (painel.produto != null) {
        print('painel.id: ${painel.id}');
        print('painel: ${painel.runtimeType}');
        painelListTemp.clear();
        if ( painelMapList[painel.produto.id] !=null) {
          
        }
        painelListTemp = painelMapList[painel.produto.id];
        painelListTemp.add(painel);
        print('painelListTemp: ${painelListTemp.length}');
        // painelMapList[painel.produto.id] = painelListTemp;
      }
    }
  }
}
